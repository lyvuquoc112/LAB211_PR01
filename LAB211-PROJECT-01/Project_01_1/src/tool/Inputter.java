...
public class Inputter {
    ...
    public Order inputOrder(Customers customers, SetMenus setMenus, boolean isUpdate, Order oldOrder) {
        ...
        do {            
            ...
            } else if (setMenuCode.length() == 5 && setMenus.searchById(setMenuCode) != null) {
                checkSetMenuCode = true;
            } else {
                if (setMenuCode.length() != 5) {
                    System.out.println("Set Menu Code must be 5 characters: ");
                } else {
        ...
        Order order;
        if (isUpdate) {
            order = new Order(oldOrder.getOrderId(), oldOrder.getCustomerCode(), setMenuCode, numberOfTable, eventDate);
        } else {
